...
--- it really sucks
local Point = require("co.geo").Point

--- @class co.window.Module
--- @field active_windows co.window.Window[]
local M = {
  active_windows = {},
}

local nsid = vim.api.nvim_create_namespace("co.window.error")
local win_valid = vim.api.nvim_win_is_valid
local buf_valid = vim.api.nvim_buf_is_valid

--- @class co.window.Config
--- @field width number
--- @field height number
--- @field row number?
--- @field col number?
...
--- @field config co.window.Config
--- @field win_id number
--- @field buf_id number
--- @field type "capture_input" | "status"

--- @class co.window.SplitWindow
--- @field win number
--- @field buffer number
...
function M.display_error(error_text)
  local window =
    create_floating_window(create_window_top_config(), " co Error ", false)
  ...
  table.insert(
    lines,
    1,
    "co: Fatal operational error encountered (error logs may have more in-depth information)"
  )
...
function M.display_cancellation_message(text)
  local config = create_transparent_top_right_config(100, " co Cancelled ")
  local window = create_floating_window(config, " co Cancelled ", false)
...
function M.display_full_screen_message(lines)
  ...
  M.clear_active_popups()
  local window =
    create_floating_window(create_window_full_screen(), " co ", true)
...
function M.create_centered_window()
  ...
  local window = create_floating_window(config, " co ", true)
...
function M.status_window()
  M.clear_active_popups()
  local config = create_transparent_top_right_config(100, " co - Status ")
  local window = create_floating_window(config, " co - Status ", false)
  window.type = "status"
  return window
end
...
--- @class co.window.SplitWindowOpts
--- @field split_direction "vertical" | "horizontal" | nil
--- @field filetype string

--- @param content string[]
---@param buffer number | nil
---@param opts co.window.SplitWindowOpts | nil
--- @return co.window.SplitWindow
